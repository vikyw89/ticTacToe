...
const player = (arg) => {
    ...
    const score = (arg) => {
        return arg
            ? document.querySelector(`.player-${_input.toLowerCase()}-score`).textContent = arg
            : document.querySelector(`.player-${_input.toLowerCase()}-score`).textContent
    }     

    const brain = (arg) => {
        return arg
            ? document.querySelector(`#player-${_input.toLowerCase()}-setting`).value = arg
            : document.querySelector(`#player-${_input.toLowerCase()}-setting`).value
    }
...
// const arg = ['1','2','3','4','5','6','7','8','9']
// const test = [...document.querySelectorAll('.cell')].map((element, index)=> element.textContent = 'X')


const board = (arg) => {
    return arg
        ? [...document.querySelectorAll('.cell')]
            .map((element, index, arr)=> element.textContent = arg[index])
        : [...document.querySelectorAll('.cell')]
